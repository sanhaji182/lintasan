<script lang="ts">
  ...
  import { Search, Copy, Play, TestTube2, Boxes, RefreshCw, Route, Server } from 'lucide-svelte';

  let rows = $state<CallableModel[]>([]);
  let loading = $state(true);
  let error = $state('');
  let query = $state('');
  let kind = $state<'all' | 'route' | 'provider'>('all');
...
<div class="catalog-page">
  <header class="catalog-header">
    <div><div class="eyebrow">CALLABLE INVENTORY</div><h2>Models</h2><p>Everything you can call now: routes and provider models. Metadata is shown only when reported by the source.</p></div>
  ...
  <section class="catalog-tools" aria-label="Model catalog filters">
    ...
    <div class="filters" role="group" aria-label="Model type">
      {#each [['all','All'], ['route','Aliases & Combos'], ['provider','Provider Models']] as option}
    ...
    <div class="model-grid">
      {#each visible as row (row.rowKey)}
        <article class="model-card">
          <div class="model-top">
            <div class="kind-icon">{#if row.kind === 'route'}<Route size={18} />{:else}<Server size={18} />{/if}</div>
            <div class="identity"><code>{row.id}</code><span>{row.kind === 'route' ? 'Alias / combo' : 'Provider model'}</span></div>
