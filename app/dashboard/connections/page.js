...
export default function ConnectionsPage() {
  ...
  const [testingConn, setTestingConn] = useState({}); // per-connection test state
  const [connTestResults, setConnTestResults] = useState({}); // per-connection test results
  ...
  async function testExistingConnection(conn) {
    setTestingConn(s => ({ ...s, [conn.id]: true }));
    setConnTestResults(s => ({ ...s, [conn.id]: null }));
    try {
      const res = await fetch("/api/connections/test", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const result = data.success
        ? { success: true, msg: data.message || "Connected", ms: data.latency_ms }
        : { success: false, msg: data.error || "Failed", ms: data.latency_ms };
      setConnTestResults(s => ({ ...s, [conn.id]: result }));
    } catch (e) {
      setConnTestResults(s => ({ ...s, [conn.id]: { success: false, msg: e.message, ms: 0 } }));
    }
    setTestingConn(s => ({ ...s, [conn.id]: false }));
  }
                      ...
                      {catPresets.map(p => (
                        <button key={p.id} onClick={() => selectPreset(p)} style={presetBtn}>
                          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                            <img
                              src={`/api/favicon?domain=${getHost(p.website)}`}
                              alt=""
                            ...
                            {p.website && (
                              <a href={p.website} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}
                                style={{ fontSize: "10px", color: "var(--fg-3)", textDecoration: "none", fontWeight: 400, marginLeft: "auto", flexShrink: 0, display: "flex", alignItems: "center", gap: "2px" }}
                                title={`Visit ${p.name}`}>
                          ...
                          <span style={{ fontSize: "11px", color: "var(--fg-3)", marginTop: "4px", display: "block" }}>{p.description}</span>
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            <div key={conn.id} style={{ ...card, marginBottom: 0 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  <button onClick={() => testExistingConnection(conn)} disabled={testingConn[conn.id]} style={{ ...btnSmall, color: testingConn[conn.id] ? "var(--fg-3)" : "var(--info)" }} title="Test connection">
                    {testingConn[conn.id] ? <IconSpinner /> : <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>}
                  </button>
              ...
              {/* Test result */}
              {connTestResults[conn.id] && (
                <div style={{ marginTop: "8px", padding: "6px 10px", borderRadius: "4px", fontSize: "12px",
                  background: connTestResults[conn.id].success ? "var(--success)" + "12" : "var(--error)" + "12",
                  border: "1px solid " + (connTestResults[conn.id].success ? "var(--success)" : "var(--error)"),
                  color: connTestResults[conn.id].success ? "var(--success)" : "var(--error)" }}>
                  {connTestResults[conn.id].success ? "✓" : "✗"} {connTestResults[conn.id].msg}
                  {connTestResults[conn.id].ms > 0 && <span style={{ marginLeft: "6px", opacity: 0.7 }}>({connTestResults[conn.id].ms}ms)</span>}
                </div>
              )}
...
// Icons
// Extract hostname safely for favicon (handles empty/relative URLs)
