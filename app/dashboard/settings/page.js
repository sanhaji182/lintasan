    ...
    <div className="fade-in">
      ...
      <div style={{ ...card, marginTop: "16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
          <div style={iconBadge}><IconBrain /></div>
          <div>
            <h2 style={{ fontSize: "14px", fontWeight: 600, color: "var(--fg-0)", margin: 0 }}>🧠 Thinking Mode</h2>
            <p style={{ fontSize: "12px", color: "var(--fg-3)", margin: 0, marginTop: "2px" }}>Control reasoning behavior for DeepSeek models (V4 Pro, R1). Mirrors 9router's thinking.type injection.</p>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "8px" }}>
          {[
            { value: "auto", label: "Auto", desc: "Model decides (default)", color: "var(--fg-2)" },
            { value: "enabled", label: "Enabled", desc: "Full reasoning (thorough)", color: "var(--primary)" },
            { value: "disabled", label: "Disabled", desc: "No reasoning (fast, direct)", color: "var(--success)" },
          ].map(opt => (
            <button key={opt.value} onClick={() => saveSetting("thinking_mode", opt.value)} style={{ padding: "10px", background: (settings.thinking_mode || "auto") === opt.value ? "var(--primary-light)" : "var(--bg-card)", border: (settings.thinking_mode || "auto") === opt.value ? "2px solid var(--primary)" : "1px solid var(--border)", borderRadius: "var(--radius-sm)", cursor: "pointer", textAlign: "center" }}>
              <p style={{ fontSize: "13px", fontWeight: 600, color: (settings.thinking_mode || "auto") === opt.value ? opt.color : "var(--fg-0)", margin: 0 }}>{opt.label}</p>
              <p style={{ fontSize: "11px", color: "var(--fg-3)", margin: "2px 0 0" }}>{opt.desc}</p>
            </button>
          ))}
        </div>
