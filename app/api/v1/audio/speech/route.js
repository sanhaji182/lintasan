...
export async function POST(request) {
  ...
  try {
    ...
    if (!voice) {
      return Response.json(
        { error: { message: "voice is required", type: "invalid_request_error", param: "voice", code: null } },
        { status: 400 }
      );
    }
    ...
    for (const conn of connections) {
      ...
      const speechPath = fullConn.speech_path || "/v1/audio/speech";
      const url = fullConn.base_url + speechPath;
      const startTime = Date.now();

      try {
        const reqBody = { model, input, voice };
        if (response_format) reqBody.response_format = response_format;
        if (speed != null) reqBody.speed = speed;
