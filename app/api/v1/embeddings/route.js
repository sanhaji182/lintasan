...
export async function POST(request) {
  ...
  try {
    ...
    const { model, input, encoding_format } = body;
    ...
    for (const conn of connections) {
      ...
      const embeddingsPath = fullConn.embeddings_path || "/v1/embeddings";
      const url = fullConn.base_url + embeddingsPath;
      const startTime = Date.now();

      try {
        const reqBody = { model, input };
        if (encoding_format) reqBody.encoding_format = encoding_format;
